...
"""Loggain fil i loggain mappen.
    Emil säger att jag borde kopierat.
    Rätt filer i rätt mappar
...
"""Tar emot innehållet från formuläret och updaterar sidan """
...
def updateRegisterUser():
    ...
    if(password1 == password2):
        ...
        if validate_email(mejl, verify = True):
            #här fanns en mysql connection innan den finns nu i metoden handelsUsers
            cursor = con.cursor()
            query = ("INSERT user(username, mejl, password1)" "VALUES(%s,%s,%s)", (username, mejl, password1));
            cursor.execute(query)
    else:
        "Ett fel har inträffat, det kan bero på att användarnamnet redan existerar eller pga du har angivit fel information"
...
""" ändra användares kan redigera sin information"""
@route ("/")    
def editUser():
    global username, mejl, password1, password2
    con = mysql.connector.connect(username=username, password1=password1, host='localhost', database='sagoland')
